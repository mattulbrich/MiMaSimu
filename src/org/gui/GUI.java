...
public class GUI extends JFrame {
                    ...
                    if (option == JOptionPane.YES_OPTION) {
                        ...
                        final FileFilter filter = new FileFilter() {
                            ...
                            public boolean accept(final File arg0) {
                                return arg0.getName().matches(".*\\.mem") || arg0.getName().matches(".*\\.mima")
                                        || arg0.isDirectory();
                            }
                        };
                        ...
                        if (fileExplorer.showOpenDialog(null) == 0) {
                            ...
                            if (!input.getName().matches(".*\\.mem") && !input.getName().matches(".*\\.mima")) {
        ...
        searchMem.addCaretListener(new CaretListener() {
            ...
            public void caretUpdate(final CaretEvent arg0) {
                if (isNumber(searchMem.getText())) {
                    memoryTable.scrollRectToVisible(memoryTable.getCellRect(getNum(searchMem.getText()), 0, false));
                }
            }
        });
    ...
    private void addLabels() {
        ...
        tmpLabel.setLocation(elements.get("akkuN").getX(), elements.get("akkuN").getY() - 15);
        elementLabels.put("lbl_akku", tmpLabel);

        tmpLabel = new JLabel("Eins");
        tmpLabel.setLocation(elements.get("eins").getX(), elements.get("eins").getY() - 15);
        elementLabels.put("lbl_eins", tmpLabel);

        tmpLabel = new JLabel("SAR");
        tmpLabel.setLocation(elements.get("sar").getX(), elements.get("sar").getY() - 15);
        elementLabels.put("lbl_sar", tmpLabel);

        tmpLabel = new JLabel("SDR");
        tmpLabel.setLocation(elements.get("sdr").getX(), elements.get("sdr").getY() - 15);
        elementLabels.put("lbl_sdr", tmpLabel);

        tmpLabel = new JLabel("X");
        tmpLabel.setLocation(elements.get("x").getX(), elements.get("x").getY() - 15);
        elementLabels.put("lbl_x", tmpLabel);

        tmpLabel = new JLabel("Y");
        tmpLabel.setLocation(elements.get("y").getX(), elements.get("y").getY() - 15);
        elementLabels.put("lbl_y", tmpLabel);

        tmpLabel = new JLabel("Z");
        tmpLabel.setLocation(elements.get("z").getX(), elements.get("z").getY() - 15);
        elementLabels.put("lbl_z", tmpLabel);

        tmpLabel = new JLabel("ALU");
        tmpLabel.setLocation(elements.get("alu").getX(),
                elements.get("alu").getY() + elements.get("alu").getHeight() / 2 - 15);
        elementLabels.put("lbl_alu", tmpLabel);

        tmpLabel = new JLabel("IR");
        tmpLabel.setLocation(elements.get("irOP").getX(), elements.get("irOP").getY() - 15);
        elementLabels.put("lbl_ir", tmpLabel);

        tmpLabel = new JLabel("IAR");
        tmpLabel.setLocation(elements.get("iar").getX(), elements.get("iar").getY() - 15);
        elementLabels.put("lbl_iar", tmpLabel);

        for (final JLabel label : elementLabels.values()) {
        ...
        tmpLabel.setLocation(elements.get("akkuBus").getX() + elements.get("akkuBus").getWidth() / 2,
                elements.get("akkuBus").getY() - 20);
        busLabels.put("lbl_akkuBus", tmpLabel);

        tmpLabel = new JLabel("<html>24 <br> / </html>");
        tmpLabel.setLocation(elements.get("einsBus").getX() + elements.get("einsBus").getWidth() / 2,
                elements.get("einsBus").getY() - 20);
        busLabels.put("lbl_einsBus", tmpLabel);

        tmpLabel = new JLabel("<html>20 <br> / </html>");
        tmpLabel.setLocation(elements.get("sarBus").getX() + elements.get("sarBus").getWidth() / 2,
                elements.get("sarBus").getY() - 20);
        busLabels.put("lbl_sarBus", tmpLabel);

        tmpLabel = new JLabel("<html>24 <br> / </html>");
        tmpLabel.setLocation(elements.get("sdrBus").getX() + elements.get("sdrBus").getWidth() / 2,
                elements.get("sdrBus").getY() - 20);
        busLabels.put("lbl_sdrBus", tmpLabel);

        tmpLabel = new JLabel("<html>24 <br> / </html>");
        tmpLabel.setLocation(elements.get("xBus").getX() + elements.get("xBus").getWidth() / 2,
                elements.get("xBus").getY() - 20);
        busLabels.put("lbl_xBus", tmpLabel);

        tmpLabel = new JLabel("<html> &#32;&#47; <br> 24 </html>");
        tmpLabel.setLocation(elements.get("yBus").getX() + elements.get("yBus").getWidth() / 2,
                elements.get("yBus").getY() - 5);
        busLabels.put("lbl_yBus", tmpLabel);

        tmpLabel = new JLabel("<html>24 <br> / </html>");
        tmpLabel.setLocation(elements.get("zBus").getX() + elements.get("zBus").getWidth() / 2,
                elements.get("zBus").getY() - 20);
        busLabels.put("lbl_zBus", tmpLabel);

        tmpLabel = new JLabel("<html>24 <br> / </html>");
        tmpLabel.setLocation(elements.get("irBus").getX() + elements.get("irBus").getWidth() / 2,
                elements.get("irBus").getY() - 20);
        busLabels.put("lbl_irBus", tmpLabel);

        tmpLabel = new JLabel("<html>20 <br> / </html>");
        tmpLabel.setLocation(elements.get("iarBus").getX() + elements.get("iarBus").getWidth() / 2,
        ...
        tmpLabel.setLocation(elements.get("sarMem").getX() - 2,
                elements.get("sarMem").getY() + elements.get("sarMem").getHeight() / 2 - 15);
        busLabels.put("lbl_sarMem", tmpLabel);

        tmpLabel = new JLabel("<html> 	&mdash; 24</html>");
        tmpLabel.setLocation(elements.get("sdrMem").getX() - 2,
                elements.get("sdrMem").getY() + elements.get("sdrMem").getHeight() / 2 - 15);
        busLabels.put("lbl_sdrMem", tmpLabel);

        for (final JLabel label : busLabels.values()) {
            label.setSize(40, 30);
            label.setVisible(true);
            label.setBackground(new Color(0, 0, 0, 0));
            this.mimaContent.add(label);
        ...
        for (final JLabel label : dataLabels.values()) {
            label.setVisible(true);
            label.setBackground(new Color(0, 0, 0, 0));
    ...
    private void addButtons() {
        ...
        tmp.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(final ActionEvent arg0) {
                final JButton tmp = buttons.get("start");
        ...
        tmp.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(final ActionEvent arg0) {
                ...
                final FileFilter filter = new FileFilter() {
                    ...
                    @Override
                    public boolean accept(final File arg0) {
                        return arg0.getName().matches(".*") || arg0.getName().matches(".*") || arg0.isDirectory();
                    }
                };
                ...
                if (fileExplorer.showOpenDialog(null) == 0) {
                    ...
                    if (!input.getName().matches(".*") && !input.getName().matches(".*")) {
    ...
    private void addBus() {
        final HashMap<String, Element> bus = new HashMap<String, Element>();
        ...
        for (final Element element : bus.values()) {
            element.setBackgroundColor(backgroundColor);
            element.setBorderColor(borderColor);
            element.setActiveColor(activeColor);
            element.setVisible(true);
    ...
    private void addArrows() {
        final HashMap<String, Element> arrowH = new HashMap<String, Element>();
        final HashMap<String, Element> arrowV = new HashMap<String, Element>();
        ...
        for (final Element element : arrowH.values()) {
        ...
        for (final Element element : arrowV.values()) {
    ...
    private void addBorder() {
        ...
        final Element tmp = new Rectangle();
        tmp.setSize(479, 415);
        tmp.setLocation(0, 0);
    ...
    public void setStatus(final boolean Ar, final boolean Aw, final boolean RX, final boolean RY, final boolean RZ,
        ...
        final LinkedList<Element> active = new LinkedList<Element>();

        for (final Element element : elements.values()) {
            element.setActive(false);
        }

        if (Ar || Aw || RX || RY || RZ || E || Pr || Pw || Ir || Iw || Dr || Dw || S)
        ...
        if (R || W) {
            ...
            final ListSelectionModel sel = memoryTable.getSelectionModel();
            sel.setSelectionInterval(getNum(elements.get("sar").getText()), getNum(elements.get("sar").getText()));
            memoryTable.scrollRectToVisible(memoryTable.getCellRect(getNum(elements.get("sar").getText()), 0, false));
        } else {
            final ListSelectionModel sel = memoryTable.getSelectionModel();
            sel.clearSelection();
        }
        ...
        for (final Element element : active) {
            if (element != null)
     ...
     * @param memory
     */
    public void setValues(final int akku, final int alu, final int iar, int ir, final int sar, final int sdr,
            final int x, final int y, final int z, final HashMap<Integer, Integer> memory) {
     ...
     * @param memory
     */
    public void deleteValues(final HashMap<Integer, Integer> memory) {
    ...
    private String toBinary(final int value, final int length) {
        ...
        if (isNumber(input)) {
            ...
            while (input.length() > 0) {
                if (input.charAt(0) <= 57)
                    number += (input.charAt(0) - 48) * Math.pow(base, input.length() - 1);
                else
                    number += (input.charAt(0) - 55) * Math.pow(base, input.length() - 1);
     ...
     */
    public void setChangedStatus(final boolean value) {
        changed = value;
    }
}
